...
import com.numb3r3.nlp.core.sentencer.local.MorphaSentencer;
 ...
 * <p/>
 * > Perl implementation of the porter stemming algorithm > described in the
 * paper: "An algorithm for suffix stripping, M F Porter" >
 * http://www.muscat.com/~martin/stem.html > > Daniel van Balen
 * (vdaniel@ldc.usb.ve) > > October-1999
 *
 * @author hideki
 */
public class PorterStemmer implements Stemmer {
    ...
    private final static String c = "(?:[^aiueoy]|(?:(?<=[aiueo])y)|\by)"; // reconoce
    // una
    // consonante

    // This is a vowel. "aiueo" and "y" if preceded by a consonant
    private final static String v = "(?:[aiueo]|(?:(?<![aiueo])y))"; // reconoce
    // una
    // vocal
    ...
    // Matches if (m > 0)
    private final static Pattern m_gt_0 = Pattern.compile("^(?:" + c + "+)?(?:"
            + v + "+" + c + "+){1,}(?:" + v + "+)?$");
    // Matches if (m > 1)
    private final static Pattern m_gt_1 = Pattern.compile("^(?:" + c + "+)?(?:"
            + v + "+" + c + "+){2,}(?:" + v + "+)?$");
    // Matches if (m = 1)
    private final static Pattern m_eq_1 = Pattern.compile("^(?:" + c + "+)?(?:"
            + v + "+" + c + "+){1}(?:" + v + "+)?$");

    // Matches *o
    private final static Pattern pO = Pattern.compile(c + v
            + "(?:[^aiueowxy])$");
    ...
    private StringWrapper step1(StringWrapper w) {
        ...
        // If 2nd or 3rd of the previous rules was successful try the extra
        // rules...

        // Si aplicaron alguna de las dos ultimas reglas de "1b" hacemos las
        // siguientes
    ...
    private StringWrapper step2(StringWrapper w) {
        ...
        // To speed up the algorithm we do a switch on the penultimate letter of
        // the
        // word being tested. Same for steps 3 and 4.
        int begin = 0;
        if (w.word.length() >= 2) {
            begin = w.word.length() - 2;
        }

        String letter = w.word.substring(begin,
                w.word.length() - 1);
        if (letter.equals("a")) {
            if (sub(w, p_ational, m_gt_0, "ate")) {
            } else if (sub(w, p_tional, m_gt_0, "tion")) {
            }
        } else if (letter.equals("c")) {
            if (sub(w, p_enci, m_gt_0, "ence")) {
            } else if (sub(w, p_anci, m_gt_0, "ance")) {
            }
            ...
            if (sub(w, p_bli, m_gt_0, "ble")) {
            } else if (sub(w, p_alli, m_gt_0, "al")) {
            } else if (sub(w, p_entli, m_gt_0, "ent")) {
            } else if (sub(w, p_eli, m_gt_0, "e")) {
            } else if (sub(w, p_ousli, m_gt_0, "ous")) {
            }
        } else if (letter.equals("o")) {
            if (sub(w, p_ization, m_gt_0, "ize")) {
            } else if (sub(w, p_ation, m_gt_0, "ate")) {
            } else if (sub(w, p_ator, m_gt_0, "ate")) {
            }
        } else if (letter.equals("s")) {
            if (sub(w, p_alism, m_gt_0, "al")) {
            } else if (sub(w, p_iveness, m_gt_0, "ive")) {
            } else if (sub(w, p_fulness, m_gt_0, "ful")) {
            } else if (sub(w, p_ousness, m_gt_0, "ous")) {
            }
        } else if (letter.equals("t")) {
            if (sub(w, p_aliti, m_gt_0, "al")) {
            } else if (sub(w, p_iviti, m_gt_0, "ive")) {
            } else if (sub(w, p_biliti, m_gt_0, "ble")) {
            }
        }
        return w;
    }
    ...
    private StringWrapper step3(StringWrapper w) {
        ...
        if (letter.equals("e")) {
            if (sub(w, p_icate, m_gt_0, "ic")) {
            } else if (sub(w, p_ative, m_gt_0, "")) {
            } else if (sub(w, p_alize, m_gt_0, "al")) {
            }
        } else if (letter.equals("i")) {
            sub(w, p_iciti, m_gt_0, "ic");
        } else if (letter.equals("l")) {
            if (sub(w, p_ical, m_gt_0, "ic")) {
            } else if (sub(w, p_ful, m_gt_0, "")) {
            }
        } else if (letter.equals("s")) {
            sub(w, p_ness, m_gt_0, "");
        }
    ...
    private StringWrapper step4(StringWrapper w) {
        ...
        if (letter.equals("a")) {
            ...
            if (sub(w, p_ance, m_gt_1, "")) {
            } else if (sub(w, p_ence, m_gt_1, "")) {
            }
        } else if (letter.equals("e")) {
            sub(w, p_er, m_gt_1, "");
        } else if (letter.equals("i")) {
            ...
            if (sub(w, p_able, m_gt_1, "")) {
            } else if (sub(w, p_ible, m_gt_1, "")) {
            }
        } else if (letter.equals("n")) {
            if (sub(w, p_ant, m_gt_1, "")) {
            } else if (sub(w, p_ement, m_gt_1, "")) {
            } else if (sub(w, p_ment, m_gt_1, "")) {
            } else if (sub(w, p_ent, m_gt_1, "")) {
            }
        } else if (letter.equals("o")) {
            ...
            if (sub(w, p_ate, m_gt_1, "")) {
            } else if (sub(w, p_iti, m_gt_1, "")) {
            }
    ...
    private StringWrapper step5(StringWrapper w) {
        ...
        if (m5_1.find()) {
            if (m_gt_1.matcher(m5_1.group(1)).find()
                    || (m_eq_1.matcher(m5_1.group(1)).find() && pO.matcher(
                    m5_1.group(1)).find())) {
                w.word = m5_1.group(1);
            }
        }
    ...
    /**
     * if($word =~ /(.+)icate$/) { if(($w=$1) =~ /$m_gt_0/o){ $word=$w."ic"; } }
     *
     * @param w
     * @param p1
     * @param p2
    ...
    public String stem(String word) {
        return stemWord(word);  //To change body of implemented methods use File | Settings | File Templates.
    }
    ...
    public String stemSentence(String sentence) {
        ...
        return Joiner.on(" ").skipNulls().join(words).trim();
    ...
    public static void main(String[] args) {
        ...
        Stemmer stemmer2 = new PorterStemmer();
        for (String sent : sentencer.sentences(test)) {

            System.out.println(sent);
            for (Iterator it = tokenizer.tokens(sent); it.hasNext(); ) {
                Token token = (Token) it.next();
                PostaggedToken postoken = new PostaggedToken(token);
                postoken.setLemma(stemmer2.stem(token.term()).toLowerCase());
